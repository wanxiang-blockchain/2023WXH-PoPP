...
library Events {
    ...
    event FinancePoolModuleWhitelisted(
        address indexed collectModule,
        bool indexed whitelisted,
        uint256 timestamp
    );
    ...
    event ProfileCreated(
        ...
        string followNFTURI,
        uint256 timestamp,
        DataTypes.ProfileType profileType
    );
     ...
     *
     * @param profileId The profile's token ID.
     * @param baseProfileId The base ID.
    ...
    //@deme
    event ProfileType(uint256 indexed profileId, DataTypes.ProfileType indexed profileType);
    //@deme
    event SetProfileAuthHubAddress(uint256 profileId, address authHubAddress);

    event SetMuteFollower(uint256 baseId, address follower, bool mute);
    event SetMutePoppProfile(uint256 baseId, uint256 poppProfileId, bool mute);
    event PlanetKickOutFollower(uint256 profileId, address follower);
}
