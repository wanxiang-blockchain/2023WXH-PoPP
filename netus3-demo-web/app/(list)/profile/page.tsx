...
const ProfileListPage = () => {
	...
	// 用户列表
	const { poppList, loading: poppLoading, createPrivate } = useProfileCreatedHooks({ isRedux })
	// 创建个人的信息
	...
	const handleSubmit = async () => {
		...
		try {
			...
			setTimeout(() => {
				toast.success(t('list.profile.craete.success'))
	...
	const handleFollowProfile = async (str: ProfileCreateListType) => {
		...
		try {
			...
			setTimeout(() => {
				toast.success(t('list.profile.follow.success'))
				console.log('receipt', receipt)
				void setRedux.toggle()
				dispatch(saveIsPend(false))
			}, SENDSUCCESSTIME)
		...
		<>
			<div className="h-full	px-5">
				<div className={'flex h-[var(--page-height-h5)] w-full md:h-[var(--page-height)]'}>
					<ScrollShadow size={100} className="h-full w-full">
						...
						{poppList.length === 0 && (
							<NoData text={t('list.profile.no.create.tips')} classOther="h-full items-center justify-center flex" />
						)}
					</ScrollShadow>
				</div>
			</div>
			...
			>
				<ModalContent>
					{onClose => (
						<>
							<ModalHeader className="flex flex-col gap-1">{t('list.profile.modal.title')}</ModalHeader>
							<ModalBody>
								<Input
									value={state.name}
									...
									type="text"
									name="name"
									label={t('list.profile.modal.name')}
